...
# 此处补全：对im1执行高斯模糊。
# 提示：可用cv.GaussianBlur # 高斯核/标准差
im_gs = blur = cv.GaussianBlur(im1, (7, 7), 0)
...
min_val, max_val, min_loc, max_loc = cv.minMaxLoc(im3)
im3 = cv.normalize(img, None, 0, 255, cv.NORM_MINMAX)
