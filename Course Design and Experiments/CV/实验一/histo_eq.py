...
def get_histogram(gray_img):
    ...
    shape_sum = gray_img.shape[0]*gray_img.shape[1]
    for i in range(gray_img.shape[0]):
        for j in range(gray_img.shape[1]):
            # ###### 1. 这里编程实现直方图统计 ##########
            Pr[gray[i][j]] = Pr[gray[i][j]]+1
    ...
    # ###### 2. 归一化直方图，获得概率分布计 ##########
    Pr_float =  np.zeros(256, np.float32)
    for i in range(0,256):
        Pr_float[i] = float(Pr[i])/shape_sum
    # ############## 结束编程 #############
    pass
